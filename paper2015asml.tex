...
\author{Arjun Menon$^{1}$, Pooja Kacker$^{2}$, Sachin Chitta$^{3}$% <-this % stops a space
\thanks{*This work was partly supported by NSF NRI Grant IIS 1317849}% <-this % stops a space
\thanks{$^{1}$Anki Robotics, San Francisco, CA - this work was performed while Arjun was at SRI International
        {\tt\small }}%
\thanks{$^{2}$Independent Consultant, Menlo Park, CA
        {\tt\small }}%
\thanks{$^{3}$Corresponding author: SRI International, Menlo Park, CA
        {\tt\small sachin.chitta@sri.com}}%
}
...
We computed the correlation between observer ratings of smoothness vs. the measured smoothness of the trajectories (Table~\ref{tab:corr_smoothness}). Note that higher values of the measured smoothness feature  indicate that the trajectories are jagged or non-smooth. The smoothness rating is not very highly correlated with any of the measured smoothness features.  A likely explanation is that excessive motions of the wrist (while still measured as smooth) are perceived as non-smooth by the human observer. We also computed the correlation between path lengths and the efficiency rating assigned by the human observer (Table~\ref{tab:path_length}). Here, the correlation was high (the negative value of the correlation coefficients indicate that higher path lengths are perceived as less efficient). 

\begin{table}
\centering
\begin{tabular}{|p{1.25cm}|c|c|c|c|}
\hline
& Smooth & Smoothness & Smoothness & Smoothness \\ 
& & (Joint) & (Tool) & (Wrist) \\ \hline
Smooth & 1 & -0.27 & -0.29 & -0.38 \\ \hline
Smoothness (Joint) & -0.27 & 1 & 0.68 & 0.62 \\ \hline
Smoothness (Tool) & -0.29 & 0.68 & 1 & 0.87 \\ \hline
Smoothness (Wrist) & -0.38 & 0.62 & 0.87 & 1 \\ \hline
\end{tabular}
\caption{Correlation between human observed rating for smoothness and measured smoothness values for the trajectories.}
\label{tab:corr_smoothness}
\vspace{-0.1in}
\end{table}
...
\hline
& Efficient & Path Length & Path Length & Path Length \\ 
&  & (Joint) & (Wrist) & (Tool) \\ \hline
Efficient & 1 & -0.79 & -0.8 & -0.76 \\ \hline
Path Length (Joint) & -0.79 & 1 & 0.89 & 0.86 \\ \hline
Path Length (Wrist) & -0.8 & 0.89 & 1 & 0.97 \\ \hline
Path Length (Tool) & -0.76 & 0.86 & 0.97 & 1 \\ \hline
\end{tabular}
\caption{Correlation between human observed rating for efficiency and measured path lengths for the trajectories.}
...
The maxmin distance to joint limits is the maximum over waypoints, of the minimum over joints of their distance to their respective closest limit (upper or lower), or $\max_{q \in \tj{}} \min_{\theta \in q} \min(|\theta-\theta_U|,|\theta-\theta_L|)$. Note that although the joint limit feature may not be immediately perceived by users, we chose to include it to see if the data might prove otherwise. 

\subsection{Comparative Trajectory Features}
\label{subsec:multi_traj_feat}
We also wanted to design features that captured the difference between the planned trajectory and an expected path for the end-effector in the environment. 

{\bf Nominal Trajectory Generation}: \hl{The expected or nominal end-effector trajectories were created using a breadth-first search in a 3D grid approximation of the planning problem. We plan the path of the end-effector as if it were a }\emph{free-standing cube} \hl{that is not constrained by the kinematics of the arm to which it is attached. The environment model was first discretized into a voxel-grid, and voxels are marked as occupied or empty. Any voxel that is partially occupied is considered to be occupied.

The breadth-first search was conducted using this 26-connected grid to find a path in the voxel grid from the start position of the end-effector of the arm (the }\emph{xyz} \hl{coordinates from forward kinematics) to the desired goal position. By ignoring orientation and using BFS, we get a piece-wise collection of straight-line paths in the }{\bf workspace} \hl{from that start to goal points.}

Next, we compute comparison features between $\xi_{\text{BFS}}$ and three workspace trajectories for the elbow ($\xi_\text{elbow}$), wrist ($\xi_\text{wrist}$) and finger-tip/tool ($\xi_\text{tip}$) links. The comparison features computed between each pair of trajectories are the Hausdorff distances~\cite{dubuisson1994modified} and Dynamic Time Warping~\cite{senin2008dynamic} distances. \hl{These features have also often been used for comparing trajectories in applications like location based services and are useful in comparing trajectories that might have different lengths or shapes.}~\cite{chen2013dynamic}.
...
\begin{figure}[h]
\centering
\begin{tabular}{ccc}%p{0.4\columnwidth}p{0.4\columnwidth}}
&&\\
\hline
...
\begin{figure}[t]
 \centering{\includegraphics[width=0.90\linewidth]{pictures/gini}}
  \caption{Random Forest: Gini index based relative importance measure.}
\label{fig:feature_importance_2}
\vspace{-0.3in} 
\end{figure}
\begin{figure}[t]
 \centering{\includegraphics[width=0.90\linewidth]{pictures/gini_boosting}}
  \caption{Boosting: Gini index based relative importance measure.}
...
should also help in the design of cost functions, especially since some of the features may not be traditionally included as part of a cost function. 
...
the ratings of the generated trajectories using inverse reinforcement learning (e.g. as in~\cite{kalakrishnanICRA2013},~\cite{abbeel2004apprenticeship}). We hope to examine whether a cost function derived in this manner will 
