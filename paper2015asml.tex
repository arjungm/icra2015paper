...
We computed the correlation between observer ratings of smoothness vs. the measured smoothness of the trajectories (Table~\ref{tab:corr_smoothness}). Note that higher values of the measured smoothness feature  indicate that the trajectories are jagged or non-smooth. The correlation between the smoothness rating and the measured smoothness is not very high, indicating that human observer perception of smoothness is different from the particular measure that we used for the robot trajectories. A likely explanation is that excessive motions of the wrist (while still measured as smooth) are perceived as non-smooth by the human observer. We also computed the correlation between path lengths and the efficiency rating assigned by the human observer (Table~\ref{tab:path_length}). Here, the correlation was high (the negative value of the correlation coefficients indicate that higher path lengths are perceived as less efficient). 

\begin{table}
\centering
\begin{tabular}{|p{1.25cm}|c|c|c|c|}
\hline
& Class & Smoothness & Smoothness & Smoothness \\ 
& Smooth & (Joint) & (Tool) & (Wrist) \\ \hline
Class Smooth & 1 & -0.27 & -0.29 & -0.38 \\ \hline
...
\begin{tabular}{|p{1.25cm}|c|c|c|c|}
\hline
& Class & Path Length & Path Length & Path Length \\ 
& Efficient & (Joint) & (Wrist) & (Tool) \\ \hline
\hline
Class Efficient & 1 & -0.79 & -0.8 & -0.76 \\ \hline
